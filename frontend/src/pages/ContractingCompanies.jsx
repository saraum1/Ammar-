    ...
    <div style={{ background: "#FAF7F0", minHeight: "100vh" }} dir="rtl">
      ...
      <div style={{ background: "white", padding: "36px 72px 28px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <h1 style={{ fontSize: 32, fontWeight: 900, margin: "0 0 4px", color: "#111827", letterSpacing: -0.5 }}>شركات المقاولات</h1>
          <p style={{ color: "#9ca3af", fontSize: 14, margin: 0 }}>مقاولون موثوقون لتنفيذ مشروع بنائك بأعلى جودة</p>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 24, padding: "0 70px 80px", maxWidth: 1200, margin: "0 auto" }}>
        {loading ? (
          ...
          <div style={{ gridColumn: "1/-1", textAlign: "center", padding: 60 }}>
            <p style={{ fontSize: 16, fontWeight: 600, color: "#9ca3af" }}>{search ? `لا توجد نتائج لـ "${search}"` : "لا توجد شركات مقاولات مسجّلة بعد"}</p>
          </div>
        ) : (
          filtered.map(c => (
...
function CompanyCard({ company, onView }) {
  ...
  return (
    <div
      onClick={onView}
      style={{ background: "white", borderRadius: 4, overflow: "hidden", border: "1px solid #E8DDD0", cursor: "pointer", transition: "border-color 0.2s, transform 0.2s", borderBottom: "3px solid transparent" }}
  ...
  return (
    <div style={{ background: "white", borderRadius: 4, overflow: "hidden", border: "1px solid #E8DDD0" }}>
      ...
      <div style={{ padding: "18px 20px" }}>
        ...
        <div style={{ height: 40, background: "#f3f4f6", borderRadius: 4, marginTop: 16 }} />
      </div>
    </div>
  );
}
